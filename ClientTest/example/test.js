...
test.describe("Todo List 기능 구현하기", () => {
    test("TC 1. 할 일이 정상적으로 생성된다.", async ({ page }) => {
        ...
        const createdTodoText = await page.$eval(
            ".todo",
            (el) => el.textContent,
        );
        console.assert(
...
test.describe("게시물 목록 페이지 테스트", () => {
    ...
    test("TC 2. 작성자 별 필터링 기능이 적절하게 동작한다.", async ({
        page,
    }) => {
        await page.goto("./base");
        await page.waitForSelector("#postList div");
    ...
    test("TC 3. 작성자 별 필터링 기능이 적절하게 동작한다.", async ({
        page,
    }) => {
        await page.goto("./base");
        await page.waitForSelector("#postList div");
    ...
    test("TC 5. 게시물에는 작성자 정보가 적절하게 포함되어있다.", async ({
        ...
        for (const postCard of postCards) {
            const title = await postCard.$eval(
                "h3",
